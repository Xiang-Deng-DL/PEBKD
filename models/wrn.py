...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):#notequal: x->relu(x)->conv(x)->bn(x)->relu(x)->out->conv(out)->out+convShortcut(x)
...
class NetworkBlock(nn.Module):
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers, stride, dropRate):#6*BasicBlock=12convs
        layers = []
